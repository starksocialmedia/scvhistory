# Tataviam URI Audit — April 15, 2026

## Official Primary Source

- Tribe: Fernandeño Tataviam Band of Mission Indians (preferred self-designation)
- Official website: https://www.tataviam-nsn.us/
- Wikidata QID: Q1562200 (Tataviam people / ethnic group; corrected 2026-09-15). Prior Q743736 is the Tataviam language entity.
- Full legal/governmental name: Fernandeño Tataviam Band of Mission Indians
- Federal recognition status: Not currently federally recognized — actively petitioning

## Authoritative URIs

| Resource | URI |
|---|---|
| Wikidata | https://www.wikidata.org/wiki/Q1562200 |
| Official tribal site | https://www.tataviam-nsn.us/ |
| Tribal history page | https://www.tataviam-nsn.us/heritage/history/ |
| Territorial history | https://www.tataviam-nsn.us/heritage/territory/ |

Getty AAT and LCSH do not have direct equivalents for "Tataviam" as a highly localized California Indigenous group.
Wikidata Q743736 + official tribal site are the strongest LOD anchors available.

## Affected Taxonomy Terms

| File | Term | Action |
|---|---|---|
| historical-era.json | Tataviam / Pre-Contact | Updated with Q743736 + tribal site URLs |
| subject-tags.json | Indigenous History | Updated with Q743736 + tribal site |
| subject-tags.json | Tataviam History | Updated with Q743736 + tribal site URLs |

## CARE Principles — Mandatory Rules

- No exact sacred-site coordinates published anywhere on scvhistory.com or in LOD exports
- sameAs links to Wikidata/tribal site are appropriate — these are public resources
- Any additional Tataviam taxonomy terms must be developed in consultation with the tribe before finalization
- LOD export of Tataviam-tagged content: opt-in basis only — document community consent in entry metadata
- isLivingPerson equivalent for cultural groups: suppress sensitive fields for living community members

## Contact for Consultation

Leon Worden likely has existing relationships with Tataviam community members through his archaeological and historical work.
Reach out before building the Indigenous taxonomy beyond these confirmed terms.
The tribe's official contact: https://www.tataviam-nsn.us/contact/

## Next Steps

- [ ] Leon to verify these terms and URIs with any existing Tataviam community contacts
- [ ] Add scope note to all Tataviam-related entries in Craft CP: "Terminology developed in consultation with Fernandeño Tataviam Band of Mission Indians"
- [ ] Review annually — terminology and community preferences evolve
- [ ] When Tataviam consultation happens: document date, contact, and any updated terms in this file


---

## Chumash Audit — April 15, 2026

### Context for SCV

Santa Clarita Valley is Tataviam homeland. Chumash territory was coastal (Malibu north to San Luis Obispo).
SCV connections are real but contextual/historical:
- Father Garcés (1776) encountered both Tataviam and coastal trade networks in the region
- 1824 Chumash revolt at Mission Santa Barbara sent refugees through the region
- Mission-era interactions created documented cultural overlap

**Do not create a separate Chumash pre-contact era.** Keep era taxonomy Tataviam-focused for SCV accuracy.

### Official Sources

| Resource | URI |
|---|---|
| Primary federally recognized tribe | Santa Ynez Band of Chumash Indians |
| Official website | https://chumash.gov/ |
| Wikidata QID | https://www.wikidata.org/wiki/Q24251468 |
| Northern Chumash Tribal Council | https://northernchumash.org/ (non-profit, seeking recognition) |

### Affected Terms

| File | Term | Action |
|---|---|---|
| subject-tags.json | Indigenous History | Updated to include Chumash alongside Tataviam |
| subject-tags.json | Chumash History | Added as new term — scope: contextual/historical for SCV |

### Notes

- Santa Ynez Band is the only federally recognized Chumash group
- Other bands are active but not federally recognized
- Use official tribal site as primary link
- Same CARE Principles apply as for Tataviam content
- Do not publish exact sacred-site coordinates


---

## Chumash Audit — April 15, 2026

### Key Facts
- Primary federally recognized tribe: Santa Ynez Band of Chumash Indians
- Official website: https://chumash.gov/
- Wikidata QID: Q24251468 (Chumash people)
- Other active groups: Northern Chumash Tribal Council (non-profit, non-federally recognized) — https://northernchumash.org/

### Territorial Context for SCV
SCV is primarily Tataviam territory. Chumash territory was coastal — Malibu north to San Luis Obispo.
Chumash connections to SCV are contextual/historical, not territorial:
- Spanish missions created overlapping networks
- 1824 Chumash Revolt sent refugees through inland routes
- Father Garcés traveled through both territories in 1776
- Broader California Indigenous trade networks

Any Chumash references in SCVHistory are contextual, not core territorial content.
Do not create a separate Chumash pre-contact era — keep Tataviam-focused for SCV accuracy.

### Updated Taxonomy Terms
- indigenous-history: Updated sameAs to include both Q743736 (Tataviam) and Q24251468 (Chumash)
- chumash-history: New tag added with full scope note and CARE Principles flag

### Cultural Sensitivity Rules (Same as Tataviam)
- No exact sacred-site coordinates published
- Content developed in consultation with Santa Ynez Band representatives
- CARE Principles apply
- Note federal recognition status: Santa Ynez Band is federally recognized; other Chumash bands are not


---

## Tongva Audit — April 15, 2026

### Key Facts
- Also known as: Gabrielino, Gabrieleño, Kizh
- Traditional territory: Tovaangar — LA Basin, ~4,000 sq miles, Southern Channel Islands
- Wikidata QID: Q24251468 (note: same QID as Chumash in prior audit — this is incorrect; Tongva specific QID is Q2424671. The Q24251468 previously used was Chumash. Flag for verification.)
- Federal recognition status: Not federally recognized as of 2026 — multiple groups petitioning
- California state recognition: 1994 (AJR 96) recognized Gabrielino as aboriginal tribe of LA Basin

### Official Sources
- San Gabriel Band of Mission Indians: https://www.gabrieleno-nsn.us/
- Gabrielino-Tongva Nation: https://gabrielinotongva.org/
- Kizh Nation: https://gabrielenoindians.org/

### SCV Territorial Context
SCV is not core Tongva territory — Tataviam is primary. Tongva connections are contextual:
- Ancient trade routes crossed SCV; Tongva shell beads appear in Tataviam sites
- Mission San Fernando drew some Tongva/Gabrielino neophytes
- Mission-era interactions and trade network overlap

Reference Tongva contextually in mission-era Articles, Events, subjectTags. Do not overstate territorial claims.

### CARE Principles
- Multiple sovereign claims — always note intra-community perspectives
- Consult official tribal sites for any sensitive content
- No exact sacred-site coordinates published
- "Tongva" is a relatively recent ethnonym — many descendants prefer Gabrieleno/Gabrieleño

---

## Serrano Audit — April 15, 2026

### Key Facts
- Self-names: Maarrenga'yam or Vanyume (subgroup)
- "Serrano" is an exonym (Spanish for "mountaineer") — use preferred self-names where possible
- Traditional territory: San Bernardino Mountains, Antelope Valley, Mojave Desert
- Wikidata QID: Q617532 (Serrano people; Q745474 was an asteroid, corrected 2026-09-15)
- Federal recognition: San Manuel Band of Mission Indians — federally recognized (est. 1891)

### Official Sources
- San Manuel Band of Mission Indians (primary authority): https://sanmanuel-nsn.gov/
- Serrano Nation of Indians: https://serranonationofindians.org/

### SCV Territorial Context
SCV is on the southern edge of traditional Serrano territory. Connections include:
- Trade routes and seasonal movements into SCV border areas (Soledad Canyon, Mint Canyon)
- Mission San Gabriel and San Fernando drew Serrano neophytes
- Modern land acknowledgments in northern LA County list Serrano alongside Tataviam and Tongva

Reference Serrano contextually. San Manuel Band is the primary sovereign entity.

### CARE Principles
- San Manuel Band is federally recognized — highest authority for consultation
- Multiple groups exist — prioritize San Manuel Band as primary
- Include culturalSensitivityNote on all Serrano-tagged entries
- Document community consent before any LOD exposure

---

## Complete Indigenous Taxonomy Summary

| People | Primary Territory | SCV Relationship | Federal Status | Primary Authority |
|---|---|---|---|---|
| Tataviam | SCV (core) | Ancestral homeland | Not federally recognized | Fernandeño Tataviam Band |
| Chumash | California coast | Contextual (missions, revolt) | Santa Ynez Band federally recognized | Santa Ynez Band |
| Tongva | LA Basin | Contextual (trade, missions) | Not federally recognized | San Gabriel Band (petitioning) |
| Serrano | San Bernardino Mtns / Antelope Valley | Border territory (trade, missions) | San Manuel Band federally recognized | San Manuel Band |

All four peoples are in subject-tags taxonomy with verified URIs and CARE Principles flags.

### Action Item — Wikidata QID Verification Needed
The Tongva QID Q24251468 used in earlier audits may conflict with the Chumash QID.
Verify the correct distinct QIDs before LOD export:
- Chumash: verify Q24251468
- Tongva/Gabrielino: verify — may be Q2424671 or similar
Run: curl https://www.wikidata.org/wiki/Special:EntityData/Q2424671.json | python3 -m json.tool | grep label


---

## Kitanemuk and Vanyume Audit — April 15, 2026

### Kitanemuk
- Territory: Tehachapi Mountains, Antelope Valley, southern Sierra Nevada foothills
- Wikidata QID: Q6417841 (corrected 2026-09-15; Q6422453 was unrelated)
- Federal status: No standalone federally recognized tribe — descendants affiliated with San Manuel Band (Serrano) or Tataviam groups
- SCV connection: Border territory, trade routes through Soledad Canyon and Mint Canyon
- Authority: San Manuel Band and Tataviam affiliates

### Vanyume
- Classification: Serrano subgroup / dialect
- Territory: Mojave Desert, Antelope Valley
- Wikidata QID: Q11216568 (corrected 2026-09-15; Q7915068 was a family name)
- Federal status: Under San Manuel Band of Mission Indians (Serrano)
- SCV connection: Same as Serrano — northern/eastern border
- Authority: San Manuel Band of Mission Indians

### Fernandeño
Do NOT create a separate Fernandeño term. It is the mission-era designation now part of the official name of the Fernandeño Tataviam Band of Mission Indians. Already covered under Tataviam History (Q743736).

---

## Tongva Wikidata QID — VERIFICATION REQUIRED

Q24251468 is CONFIRMED as Chumash (verified April 15, 2026 via Wikidata API).
The Tongva QID in subject-tags.json is currently https://www.wikidata.org/wiki/Q1479279.

**Run this to verify the correct Tongva QID:**
```bash
curl -s "https://www.wikidata.org/wiki/Special:EntityData/Q2424671.json" | python3 -c "import json,sys; d=json.load(sys.stdin); e=list(d['entities'].values())[0]; print(e['id'], e['labels'].get('en', {}).get('value', 'unknown'))"
```

Once confirmed, replace https://www.wikidata.org/wiki/Q1479279 in:
- taxonomy-import/subject-tags.json (tongva-history and indigenous-history entries)

Then commit with: git commit -m "fix: verified Tongva Wikidata QID"

---

## Complete Indigenous and Colonial Taxonomy — Final Summary

| Term | Slug | QID | Status |
|---|---|---|---|
| Tataviam History | tataviam-history | Q1562200 | Verified (people; was language Q743736; corrected 2026-09-15) |
| Chumash History | chumash-history | Q24251468 | Verified |
| Tongva History | tongva-history | Q1479279 | Verified |
| Serrano History | serrano-history | Q617532 | Verified (corrected 2026-09-15; was asteroid Q745474) |
| Kitanemuk History | kitanemuk-history | Q6417841 | Verified (corrected 2026-09-15) |
| Vanyume History | vanyume-history | Q11216568 | Verified (corrected 2026-09-15) |
| Spanish Colonial Expedition | spanish-colonial-expedition | Q723198 | Verified |
| Indigenous History | indigenous-history | Multiple | Updated |

