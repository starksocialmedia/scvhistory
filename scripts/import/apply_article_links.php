 ...
 *
 * relatedArticles is symmetric in meaning but stored one way round in Craft, so
 * each confirmed pair is written on both records: a reader on either piece
 * should see the other. The link in the source text is directional, but the
 * relation it evidences is not.
 *
 * A relation already present is left alone, which makes a second run a no-op.
 *
...
foreach ($decisions as $d) {
    ...
    $a = (int)($d['source'] ?? $d['a'] ?? 0); $b = (int)($d['target'] ?? $d['b'] ?? 0);
...
foreach ($add as $id => $targets) {
    $e = \craft\elements\Entry::find()->id($id)->section('articles')->status(null)->one();
    if (!$e) { echo '#' . $id . ' is not an article record' . PHP_EOL; continue; }
    if (!$hasField($e, 'relatedArticles')) { echo $e->title . ': no relatedArticles field' . PHP_EOL; continue; }
    ...
    if ($APPLY) {
        $e->setFieldValue('relatedArticles', array_merge($current, $new));
...
else { echo 'Re-render /graph to see them: an article relation joins two subjects' . PHP_EOL
     . 'that previously only shared a page.' . PHP_EOL; }
