...
return function (array $cfg): void {
    ...
    /* ------------------------------------------------------------ scope */
    $collOf = []; $collSlug = []; $frozenSkipped = 0; $frozenList = [];
    foreach (Entry::find()->section('collections')->status(null)->limit(null)->all() as $c) {
        ...
        if ($hasFrozen && $c->getFieldValue('collectionFrozen')) {
            ...
            $frozenList[] = $c->slug . ' (' . count($ids) . ' pieces)';
            continue;
        }
        foreach ($ids as $id) { $collOf[(int)$id] ??= (int)$c->id; }
    }
    /* Always said, including when it is nothing, so a run's output shows the
       freeze was checked rather than leaving that to be inferred from silence. */
    echo 'frozen collections skipped: ' . ($frozenList ? implode(', ', $frozenList) . ', ' . $frozenSkipped . ' pieces in all' : 'none') . PHP_EOL;
    echo 'pieces in scope: ' . count($collOf) . PHP_EOL;
    ...
    $md = ['# Quality pass: ' . $label, '',
           ...
           'Frozen collections skipped: ' . ($frozenList ? implode(', ', $frozenList) : 'none') . '.', '',
           '| class | before | after |', '|---|---:|---:|'];
