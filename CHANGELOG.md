...
- Agent: Claude Code
- Date: 2026-09-25
- Done: the family display rule (commit 3170876): kinship publishes only when both people are historical, nothing is derived (no siblings from shared parents, no grandparents, no grandchildren), live records only, a living person in Other relations shows as a name alone, and a box with no kinship is headed Related people. import_ruiz_census.php now refuses any relation with a living or undetermined party, in the plan and against the saved records, and reads its relations back. Audit of status(null) across the public templates. The Mentry sources extracted verbatim from Reggie into inventory/legacy/mentry-sources.json, and four scripts written and dry-run: import_mentry_sources.php (2 photographs, 8 documents, 9 scans), add_document_collection_field.php, set_mentryville_members.php, and the transcription and interpretation rule in generate_data_model.php, regenerated into docs/DATA-MODEL.md.
- Decisions: historical means a casualty record, a death date, a live obituary or a birth year more than 120 years ago, and undetermined counts as living. sw_petermentre is four printed items with four clippings, so four documents. The death certificate record has an empty body because the legacy page never transcribed it; the essay is interpretation and goes to webmasterNoteTop. subjectPerson means the subject, so the 1886 and 1899 items (Peter) and the 1954 item (Arthur) carry no subject until those people have records. The pico.htm index is the membership authority; the breadcrumb only reports differences.
- Blockers: documents cannot join a collection until add_document_collection_field.php is applied. No ch1070 master exists on Reggie. The mirror holds a second _large master, penpictureslacounty_large.jpg, beyond as0001. Only 15 of the 144 pages pico.htm lists are in Craft. The breadcrumb scan finds 13 candidates, not the 50 expected; the method is in the report.
- Result: the Acosta and Smyth kinship is withdrawn from five pages; del Valle parent and child relations stay. No disabled record leaks today (0 disabled entries), but about 60 public status(null) sites would leak a disabled record's name and link, and the homepage recent list pulls from every section including fixes and roles.
- Next: the Lyon, Wiley and Jenkins exploration event, the four-scope priority claim, person records for Sanford Lyon and William Jenkins, then the Alex Mentry #18648 profile. Candidate records surfaced: Peter Mentre (d. 1886), Arthur Charles Mentry (d. 1954), Demetrius G. Scofield; organizations Los Angeles Times, Warren Times Mirror, The Lewis Publishing Co.

2026-09-25

- Agent: Claude Code
- Date: 2026-09-25
- Done: the five quality passes applied in order (nav junk, byline dates, drop caps, footnotes, images), the photograph relation rebuild applied on title-and-caption evidence at 229 records and 404 relations, officeHolding and the jurisdiction fields applied and the four board fields retired, the review screens moved out of the web root behind a pre-deploy exposure gate, staging refreshed and verified, templates/documents/_entry.twig built, the Vasquez body de-duplicated and the create script that caused it fixed, the council roster surveyed out of the 106 articles, the City Clerk request drafted, and nine City Clerk PDFs fetched with provenance into inventory/legacy/elections-manifest.json. poppler and tesseract installed; all seven canvasses now open.
- Decisions: evidence per date rather than per term, so startEvidence and endEvidence are separate values. Name matching requires full first names and initials-only goes to the queue, after my surname-plus-initial rule merged Clyde Smyth with Cameron Smyth, father and son. A losing candidate gets a person record on two triggers, stood more than once or appears anywhere else in the archive, not on a count: a threshold misses Michael Cruz (2006, a 2021 CVRA plaintiff), Lynne Plambeck (2006) and Maria Gutzeit (2008, later a water board). Turnout and VBM are computed at render, never stored. The A/B agreement rule was dropped entirely rather than kept as a weak signal, because both routes read the same caption text, which is one source counted twice. 1992 is surfaced on the elections index rather than buried in a chart: 32.2% turnout, roughly double every other April cycle, because Measure A-92 growth control was on the ballot, and the measure lost 10,332 to 7,960.
- Blockers: the three approved record types, election, candidacy and sourceFault, are not built; the schema dry run is the next artifact and goes in with APPLY false. Swearing-in dates for 1987 through 2010 need council minutes, and both routes are shut: iqm2 times out, citydocs is behind Cloudflare. The 2024 canvass holds District 1 only, so District 3 is not among the nine and has to be asked for. The 2012 resolution gives votes cast but no registration figure, so that April turnout cannot be computed from the document we hold. inventory/legacy/elections-manifest.json is still uncommitted.
- Result: only one of the seven canvasses names winners. The 2012 scan, OCR'd, is Resolution 12-9, and section 4 declares Bob Kellar and TimBen Boydston each elected "for the full term of four years", against candidates Ed Colley, Bob Kellar, Jon Hatami, Laurie Ender and TimBen Boydston, on 3,783 votes cast at precincts plus 11,607 mail and provisional, 15,390 in all, adopted 24 April 2012. 2014 is the City's own results by precinct: 13 candidates, Acosta, Bull, Chowdhury, Conn, Daniels, Ferdman, Harper, Gutzeit, Gonzalez-Harte, McLean, Mercado-Fortine, Weste, Wieczorek, registration 111,661, 15,871 ballots, 14.2% printed on the page, and no declaration. 2016, 2018, 2020, 2022 and 2024 are all County Statements of Votes Cast, vote counts by precinct with no winner language anywhere in them, which means every November winner is still uncited and the certified source for those is a resolution we have not got. Turnout, registration then ballots then turnout then vote-by-mail then VBM share: 2016 117,972 / 90,947 / 77.1% / 37,624 / 41.4%; 2018 125,206 / 85,607 / 68.4% / 44,173 / 51.6%; 2020 142,880 / 121,458 / 85.0% / 93,501 / 77.0%; 2022 145,251 / 79,288 / 54.6% / 61,053 / 77.0%; 2024 District 1 only 23,054 / 15,508 / 67.3% / 10,409 / 67.1%. Against April at 14.2% to 19.2%, and 1992's 32.2%, consolidation roughly quadrupled turnout, with one caveat the page has to carry: a November ballots-cast figure counts everyone who voted in the general election, not everyone who marked the council race, so it measures the electorate the council race was moved into rather than interest in the council race. Two numbers are OCR readings and not yet read off the PDF by eye: the 2022 precinct subtotal, which OCR gave as 78,285 and which reconciles to 18,235 because 79,288 minus 61,053 is 18,235, and the 2020 subtotals, which do reconcile as printed. CVRA, verified against primary sources: the second complaint was filed 29 December 2021, plaintiffs and counsel as Nathan gave them, settled for $300,000 plus $70,000, $370,000 in all, a five-district map due by 30 June 2023, districts from 2024, two seats in November 2024 and three seats remaining at-large until November 2026. Not verified by anything fetched: the first lawsuit, its 2013 filing and June 2014 settlement, which the 2021 complaint does not mention and the City's own page does not mention either; that the terms of sitting members were extended by that settlement; that cumulative voting was agreed and never implemented; and McLean as the lone dissent. Those four are Nathan's account and are held as unsourced until a document carries them. Michael Cruz, the 2006 candidate against the 2021 plaintiff: undetermined, and it goes to the entity queue rather than being guessed.
- Next: the schema dry run for election, candidacy and sourceFault, then the layout proposal revised for the three eras, at-large April 1987 to 2014, at-large November 2016 to 2022, and the district transition from 2024, with the page explaining on its own face why the system changed twice. Still unread and worth chasing: the resolutions declaring the 2016, 2018, 2020, 2022 and 2024 results, the 2024 District 3 canvass, the first CVRA complaint and its settlement agreement, and council minutes for swearing-in dates 1987 through 2010.
...
- Agent: Claude Code
- Date: 2026-09-19
- Done: the caption script and both templates that print captions, the inline image float and a sitewide lightbox, the footnotes table with its three render states, the collection lander and the unflagged collection brought onto the article's design language, advanced search with facets, and the page scale measured rendered and corrected. Full report at web/review/session-2026-09-19-captions-images-footnotes-search.md.
- Decisions: box-sizing settled by matching the design rather than explaining the difference, so every wrapper is now 1320 under border-box, which is the design's 1240 of content. The three scan credits stay in finePrint. Mid-body bracketed BACK removal is an accepted rule. A caption made only of navigation leaves the asset with no caption rather than a false one. The lander keeps record/cite rather than the article's fork, because unifying it would undo the fork.
- Blockers: the footnote fields do not exist yet, so the held and elsewhere render states are written but untested; only the orphan path runs live. The caption script is a dry run; Nathan applies it.
- Result: 5,220 caption records, 794 of them navigation, 1,405 distinct files with a real caption, and 89 of our 568 assets changed because the rest of the images are not fetched. Two of my own rules were wrong and were corrected by reading the output rather than the counts: longest-variant-wins gave a body paragraph in place of a caption, and filename-matching suppression threw away "Pedro Fages". The article body was a flex column, so no inline image had been floating at all. 238 records carry a footnote marker, 2 hold the notes, 12 point at another record, 224 are orphans. 238 was my first figure, I corrected it to 229, and the correction was wrong: the extraction breaks a marker across three lines and prose.twig rejoins it at render time, so a scan of the raw field misses it. The same bug meant the orphan line rendered nowhere, and I had reported it verified after matching a CSS rule name rather than the section. Search verified live: photographs 73, articles 5, together 78, photographs with Newhall 25. Rendered measurement: the design is 1320/840/344/56, ours was 1240/760/344/56, and now matches. The real defect was inside the column: max-width in em resolves against each element's own font size, so the design's 34em renders the lead at 816, the body at 595 and the signature at 544, three right edges stepping down the page. The measure is gone rather than retuned: the prose fills its column on every record type, every block in the article column ends at the same edge, and the only space left between the prose and the sidebar is the 56px gutter. A departure from the design file, deliberate, because nothing else in the column stops where the paragraphs stopped.
- Next: the photographs and documents templates are still stubs, the 13 long paragraphs and the fidelity audit across the 1,544 photographs are not started, and the caption script needs re-running after every image import.
...
create_records_from_review.php now resolves intoKey merges, where a decision
merges into another decision in the same file rather than into an existing
record. The target has no id until the run creates it, so those are deferred
and resolved after the plan is built; chains are followed with a visited guard.
A hand-edited title now keeps the queue's spelling as an alias, since that is
